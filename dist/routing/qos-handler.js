...
exports.QoSHandler = void 0;
const events_1 = require("events");
const types_1 = require("../types");
class QoSHandler extends events_1.EventEmitter {
    constructor() {
        super();
        this.inflightQoS1 = new Map(); // key: clientId:packetId
        this.inflightQoS2 = new Map(); // key: clientId:packetId
        this.qos2Received = new Map(); // key: clientId:packetId
        this.retryInterval = null;
        this.retryTimeoutMs = 5000; // 5 seconds
        this.maxRetries = 3;
     ...
     */
    publishQoS0(message, subscribers) {
        // Fire and forget - no acknowledgment required
        for (const subscriber of subscribers) {
            const deliveryMessage = {
                ...message,
                qos: Math.min(message.qos, subscriber.qos) // Downgrade QoS if needed
            };
     ...
     */
    async publishQoS1(message, subscribers) {
        const promises = [];
        for (const subscriber of subscribers) {
            const effectiveQoS = Math.min(message.qos, subscriber.qos);
            if (effectiveQoS === types_1.QoSLevel.AT_MOST_ONCE) {
                // Downgrade to QoS 0
            ...
            const promise = new Promise((resolve, reject) => {
                ...
                const ackHandler = (clientId, packetId) => {
     ...
     */
    async publishQoS2(message, subscribers) {
        const promises = [];
        for (const subscriber of subscribers) {
            const effectiveQoS = Math.min(message.qos, subscriber.qos);
            if (effectiveQoS === types_1.QoSLevel.AT_MOST_ONCE) {
            ...
            else if (effectiveQoS === types_1.QoSLevel.AT_LEAST_ONCE) {
                // Downgrade to QoS 1
     ...
     */
    async handleQoS2Flow(clientId, message) {
        return new Promise((resolve, reject) => {
            ...
            const flowHandler = (receivedClientId, receivedPacketId, type) => {
     ...
     */
    handleAcknowledgment(packetId, clientId, type) {
     ...
     */
    handleQoS2Publish(clientId, packetId, message) {
...
exports.QoSHandler = QoSHandler;
//# sourceMappingURL=qos-handler.js.map